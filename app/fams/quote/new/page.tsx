...
function QuotationGeneratorContent() {
  ...
  const quoteId = searchParams.get('id'); // Load existing quotation
          ...
          fetch(`/api/quotations/${quoteId}`)
            ...
            .then(quote => {
              if (quote && !quote.error) {
                setQuoteDate(new Date(quote.date).toISOString().split('T')[0]);
                setItems(quote.items);
                const foundClient = clientList.find((c: any) => c.id === quote.clientId);
                if (foundClient) {
  ...
  const handleSaveQuotation = async () => {
    ...
    if (totalAmount <= 0) {
      alert("Quotation total must be greater than zero.");
      return;
    }
    ...
    try {
      ...
      setIsSaved(true);
      alert(quoteId ? "Quotation updated successfully." : "Quotation created and saved successfully.");
      
      if (!quoteId) {
        router.push('/fams/quote');
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 font-sans">
      ...
      <div className="max-w-4xl mx-auto mb-8 print:hidden flex flex-wrap gap-4 items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <Link href={`/fams/quote`} className="flex items-center text-slate-500 hover:text-slate-700 font-medium">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button 
            onClick={() => {
              ...
              const text = `*QUOTATION*%0A*From:* Get Legal Solution%0A*To:* ${clientData?.name || 'Client'}%0A*Date:* ${quoteDate}%0A------------------------%0A${itemList}%0A------------------------%0A*Total Amount: Rs. ${totalAmount.toLocaleString()}*%0A%0A_Please find the detailed PDF attached. The validity of this quotation is 15 days._`;
      ...
      <div id="printable-invoice" className={`max-w-[210mm] mx-auto bg-white min-h-[297mm] print:min-h-0 shadow-2xl p-8 print:p-0 flex flex-col relative`}>
        ...
        <div className={`text-center ${isVeryCompact ? 'mb-2' : isCompact ? 'mb-4' : 'mb-6'}`}>
          <h2 className={`${isVeryCompact ? 'text-lg' : isCompact ? 'text-xl' : 'text-2xl'} font-black text-slate-200 tracking-widest uppercase`}>Quotation</h2>
        </div>

        {/* Client & Invoice Meta */}
        <div className={`flex justify-between items-start text-sm ${isVeryCompact ? 'mb-2' : isCompact ? 'mb-4' : 'mb-8'}`}>
          <div className="w-1/2">
            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Prepared For</h3>
          ...
          <div className="w-1/3 space-y-2 text-right">
            ...
            <div>
              ...
              <input type="date" value={quoteDate} onChange={e => setQuoteDate(e.target.value)} className="w-full text-right font-bold text-slate-800 outline-none border-b border-dashed border-slate-300 focus:border-amber-500 bg-transparent text-sm" />
            </div>
          </div>
        </div>
        ...
        <div className={`mt-auto text-slate-500 ${isVeryCompact ? 'pt-2 text-[9px]' : isCompact ? 'pt-4 text-[10px]' : 'pt-8 text-xs'}`}>
          <div className={`text-center border-b border-slate-200 ${isVeryCompact ? 'pb-2 mb-2' : isCompact ? 'pb-3 mb-3' : 'pb-4 mb-4'}`}>
            <p className="font-bold text-slate-600 italic">This is a computer-generated quotation and does not require a physical signature.</p>
          </div>
          
          <div className={`text-center ${isVeryCompact ? 'text-[8px]' : 'text-[10px]'}`}>
            <p>The validity of this quotation is 15 days.</p>
          </div>
        </div>
